...
async def test_snis(cli):
    ...
    config = {
        "snis": [
            {"name": "a1.example.com", "certificate": {"id": c1["id"]}},
            {"name": "a2.example.com", "certificate": {"id": c2["id"]}},
        ]
    }
    ...
    assert len(snis) == 2
    snis = {sni.data["name"]: sni.data["certificate"] for sni in snis}
    expected = {sni["name"]: sni["certificate"] for sni in config["snis"]}
